...
├── workloads/
│   ├── job/                         # JOB .sql files (not on GitHub — see below)
│   ├── tpch/                        # generated by setup script
│   └── tpcds/                       # generated by setup script
├── schemas/
│   ├── imdb_schema.py
│   ├── tpch_schema.py
│   └── tpcds_schema.py
...
The following are excluded from version control because of size or licensing:

| Path | Reason | How to get it |
|---|---|---|
| `databases/*.duckdb` | Large binary files (1–5 GB each) | Generated/loaded via setup scripts |
| `workloads/job/*.sql` | JOB workload SQL files | Download from the JOB repository |
| `databases/imdb_csvs/` | Raw IMDB CSV dump | Download from academic sources |

The `.gitignore` already excludes `databases/*.duckdb`.
...
TPC-H takes a few seconds. TPC-DS takes 3–5 minutes.
...
**Download the JOB workload SQL files:**
- Source: [https://github.com/gregrahn/join-order-benchmark](https://github.com/gregrahn/join-order-benchmark)
- Copy the `.sql` files into `workloads/job/`
...
Optional: `--seed` (default 42) for reproducibility.
...
Same-DB experiments (imdb_to_imdb, tpch_to_tpch, tpcds_to_tpcds) serve as a control — the synthesised workload targets the same database the original was written for, giving an upper bound on how close W' can get to W.
