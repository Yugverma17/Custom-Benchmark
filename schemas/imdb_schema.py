FK_EDGES = [
    ('cast_info',       'movie_id',          'title',          'id'),
    ('cast_info',       'person_id',         'name',           'id'),
    ('cast_info',       'person_role_id',    'char_name',      'id'),
    ('cast_info',       'role_id',           'role_type',      'id'),
    ('aka_title',       'movie_id',          'title',          'id'),
    ('aka_title',       'kind_id',           'kind_type',      'id'),
    ('movie_companies', 'movie_id',          'title',          'id'),
    ('movie_companies', 'company_id',        'company_name',   'id'),
    ('movie_companies', 'company_type_id',   'company_type',   'id'),
    ('movie_info',      'movie_id',          'title',          'id'),
    ('movie_info',      'info_type_id',      'info_type',      'id'),
    ('movie_info_idx',  'movie_id',          'title',          'id'),
    ('movie_info_idx',  'info_type_id',      'info_type',      'id'),
    ('movie_keyword',   'movie_id',          'title',          'id'),
    ('movie_keyword',   'keyword_id',        'keyword',        'id'),
    ('movie_link',      'movie_id',          'title',          'id'),
    ('movie_link',      'link_type_id',      'link_type',      'id'),
    ('complete_cast',   'movie_id',          'title',          'id'),
    ('complete_cast',   'subject_id',        'comp_cast_type', 'id'),
    ('aka_name',        'person_id',         'name',           'id'),
    ('person_info',     'person_id',         'name',           'id'),
    ('person_info',     'info_type_id',      'info_type',      'id'),
    ('title',           'kind_id',           'kind_type',      'id'),
]

TABLE_SIZES = {
    'cast_info'      : 36244344,
    'movie_info'     : 14835720,
    'person_info'    :  2963664,
    'movie_keyword'  :  4523930,
    'title'          :  2528312,
    'name'           :  4167491,
    'movie_companies':  2609129,
    'aka_name'       :   901343,
    'movie_info_idx' :  1380035,
    'char_name'      :  3140339,
    'aka_title'      :   361472,
    'company_name'   :   234997,
    'keyword'        :   134170,
    'movie_link'     :    29997,
    'complete_cast'  :   135086,
    'info_type'      :      113,
    'comp_cast_type' :        4,
    'company_type'   :        4,
    'kind_type'      :        7,
    'link_type'      :       18,
    'role_type'      :       12,
}

AVG_ROW_SIZE = {
    'title'          : 120,
    'cast_info'      :  60,
    'name'           : 110,
    'movie_info'     : 200,
    'movie_info_idx' :  80,
    'movie_keyword'  :  30,
    'movie_companies':  80,
    'movie_link'     :  40,
    'person_info'    : 250,
    'aka_title'      : 150,
    'aka_name'       : 120,
    'keyword'        :  80,
    'company_name'   : 100,
    'char_name'      : 100,
    'complete_cast'  :  40,
    'info_type'      :  30,
    'comp_cast_type' :  20,
    'company_type'   :  20,
    'kind_type'      :  20,
    'link_type'      :  20,
    'role_type'      :  20,
}

PRED_COLS = {
    'title'          : [('production_year', 'int',     1880, 2019)],
    'movie_info'     : [('info_type_id',    'int',     1,    110)],
    'movie_info_idx' : [('info_type_id',    'int',     1,    110)],
    'cast_info'      : [('nr_order',        'int',     0,    1000)],
    'movie_companies': [('note',            'varchar', None, None)],
    'company_name'   : [('country_code',    'varchar', None, None)],
    'name'           : [('name',            'varchar', None, None)],
    'keyword'        : [('keyword',         'varchar', None, None)],
    'movie_keyword'  : [('keyword_id',      'int',     1,    134170)],
    'person_info'    : [('info_type_id',    'int',     1,    110)],
    'char_name'      : [('name',            'varchar', None, None)],
    'aka_name'       : [('name',            'varchar', None, None)],
    'aka_title'      : [('title',           'varchar', None, None)],
    'info_type'      : [('info',            'varchar', None, None)],
    'kind_type'      : [('kind',            'varchar', None, None)],
    'link_type'      : [('link',            'varchar', None, None)],
    'role_type'      : [('role',            'varchar', None, None)],
    'comp_cast_type' : [('kind',            'varchar', None, None)],
    'company_type'   : [('kind',            'varchar', None, None)],
}

AGG_COL = {
    'title'       : 'title',
    'name'        : 'name',
    'company_name': 'name',
    'keyword'     : 'keyword',
    'char_name'   : 'name',
    'movie_info'  : 'info',
    'info_type'   : 'info',
}
DEFAULT_AGG_COL = 'id'
